...
test_that("anything not OK explains itself and suggests a remedy", {
  ...
  fit <- suppressWarnings(lum_model(y ~ x1 + (1 | subj), data = dd, family = "multinomial", verbose = FALSE))
...
test_that("the latent budget uses weighted information, not row count", {
  ...
  dd$w <- rep(5L, nrow(dd))
  f2 <- lum_model(y ~ x1 + (1 | subj), data = dd, family = "multinomial", weights = w, verbose = FALSE)
...
test_that("a starved design is caught before fitting", {
  ...
  re <- lum_norm_re(list(subj = dd$subj), nrow(dd))
  st <- list(subj = list(type = "us", rank = NA_integer_, d_cor = TRUE))
  ck <- lum_precheck(as.integer(dd$y), 4L, re, st)
  expect_true(any(ck$status %in% c("WARN", "FAIL")))
})

test_that("lum_print_checks produces output", {
  fit <- fit_basic()
  expect_output(lum_print_checks(fit$checks, "checks"), "ok")
})
