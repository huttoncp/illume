...
test_that("fewer than three categories is refused", {
  ...
  expect_error(ilm_model(y2 ~ x1 + (1 | subj), data = dd, family = "multinomial", verbose = FALSE),
               "at least 3")
})
...
test_that("a model with no random terms is allowed", {
  ...
  dd <- sim_mlmm(seed = 2)
  f <- ilm_model(y ~ x1, data = dd, family = "multinomial", verbose = FALSE)
...
test_that("formula interface parses intercepts, slopes and smooths", {
  ...
  f2 <- ilm_model(y ~ x1 + time + (1 + time | subj), data = dd, family = "multinomial", verbose = FALSE)
  expect_equal(unname(f2$dk), 2L)                 # intercept + slope
  expect_true(!is.null(f2$Sigma_d$subj))
  expect_equal(f2$Sigma_d$subj[1, 1], 1)          # scale fixed, see ilm_mkLd()

  skip_if_not_installed("mgcv")
  f3 <- ilm_model(y ~ x1 + s(xs, k = 6) + (1 | subj), data = dd, family = "multinomial", verbose = FALSE)
  ...
  expect_error(ilm_model(y ~ x1 + mgcv::s(xs, k = 6) + (1 | subj), data = dd, family = "multinomial",
