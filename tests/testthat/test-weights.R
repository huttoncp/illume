test_that("a weighted fit equals the expanded fit exactly", {
  ...
  f_exp <- lum_model(y ~ grp + (1 | subj), data = dd2, family = "multinomial", verbose = FALSE)
  f_agg <- lum_model(y ~ grp + (1 | subj), data = ag, family = "multinomial", weights = w, verbose = FALSE)
...
test_that("weights that look like sampling weights are flagged", {
  ...
  f <- lum_model(y ~ x1 + (1 | subj), data = dd, family = "multinomial", weights = wn, verbose = FALSE)
  ...
  f2 <- lum_model(y ~ x1 + (1 | subj), data = dd, family = "multinomial", weights = wf, verbose = FALSE)
...
test_that("genuine replicate counts pass the check", {
  ...
  f <- lum_model(y ~ x1 + (1 | subj), data = dd, family = "multinomial", weights = wc, verbose = FALSE)
...
test_that("negative or mis-sized weights are refused", {
  ...
  expect_error(lum_fit(X, as.integer(dd$y), 3L, list(subj = dd$subj), family = "multinomial",
