test_that("a single core returns no pool and runs sequentially", {
  expect_null(lum_pool(1L))
  expect_equal(lum_lapply(NULL, 1:3, function(i) i * 2), list(2, 4, 6))
})

test_that("consistency checking runs and reports a refit rate", {
  fit <- fit_basic()
  cc <- lum_consistency(fit, B = 6L, verbose = FALSE)
...
test_that("get_coef and set_coef round-trip, and theta actually propagates", {
  ...
  fit <- fit_basic()
  b0 <- get_coef.lum_model(fit)
  expect_identical(b0, coef(fit, full = TRUE))
  p0 <- get_predict.lum_model(fit, marginal = FALSE)$estimate
  expect_equal(get_predict.lum_model(set_coef.lum_model(fit, b0), marginal = FALSE)$estimate, p0)

  bp <- b0; i <- grep("^subj:", names(bp))[1]; bp[i] <- bp[i] + 0.3
  pp <- get_predict.lum_model(set_coef.lum_model(fit, bp), marginal = TRUE, ndraw = 40)$estimate
  p0m <- get_predict.lum_model(fit, marginal = TRUE, ndraw = 40)$estimate
  expect_gt(max(abs(pp - p0m)), 1e-6)
})

test_that("get_predict returns long format with rowid and group", {
  fit <- fit_basic()
  g <- get_predict.lum_model(fit, marginal = FALSE)
...
test_that("model_performance returns the indices it promises", {
  fit <- fit_basic()
  mp <- model_performance.lum_model(fit, verbose = FALSE)
...
test_that("parametric bootstrap runs and respects its p-value floor", {
  fit <- fit_basic()
  r <- lum_pb_lrt(fit, "x1", B = 8L, verbose = FALSE)
