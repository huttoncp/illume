...
#' specified model. Use [lum_rqr_test()], which builds its reference by
...
#' @seealso [lum_rqr_test()], [lum_appraise()].
#' @export
lum_rqr <- function(object, conditional = TRUE, seed = 1L) {
  set.seed(seed)
  P <- lum_fitted(object, conditional); y <- object$y
  N <- nrow(P)
...
## simulated from the fit -- which is what lum_rqr_test() does, and why the
...
#' @keywords internal
#' @noRd
lum_sim_cond <- function(object, B, seed = 1L) {
  set.seed(seed)
  fam <- object$family
  eta <- lum_eta_hat(object, TRUE)
  N <- nrow(eta)
...
#'
#' Multinomial-only. [lum_sim_cond()] is the family-aware entry point.
#'
...
#' @keywords internal
#' @noRd
lum_sim_from_P <- function(P, B, seed = 1L) {
...
#' @export
lum_calibration <- function(object, nbins = 10L, B = 200L, seed = 1L) {
  ...
  if (object$C < 2L)
    stop("lum_calibration() applies to the multinomial family; for ",
         if (!is.null(object$family)) object$family$name else "this family",
         " use the residual panels in lum_appraise()", call. = FALSE)
  P <- lum_fitted(object, TRUE); y <- object$y; J <- object$J
  ys <- lum_sim_from_P(P, B, seed)
...
#' [lum_appraise()] is annotated accordingly.
#'
#' @param object A fitted `"lum_model"` object.
...
#' @export
lum_re_mahalanobis <- function(object, term = NULL) {
  ...
  B <- lum_Bhat_term(object, k); nl <- object$nlk[k]; w <- object$wk[k]
...
#' exactly uniform (see [lum_rqr()]), so a correctly specified model is not
...
#' [lum_check_covariate()] or [lum_check_omitted()] to point the test at
...
#' @export
lum_rqr_test <- function(object, B = 30L, ncores = 1L, seed = 1L,
                          stat = function(u) mean(u), verbose = TRUE) {
  obs <- stat(lum_rqr(object, TRUE, seed))
  ys <- lum_sim_cond(object, B, seed + 1L)
  X <- object$X; J <- object$J; rl <- lum_re_list_of(object); rs <- object$re_struct
  ...
  one <- function(b) {
    ...
    stat(lum_rqr(f, TRUE, seed = seed + 1000L + b))
  }
  null <- unlist(lum_lapply(cl, seq_len(B), one))
...
#'   distances.
#' @seealso [lum_check_ar()] for temporal correlation,
#'   [lum_check_omitted()] for omitted variables.
#' @export
lum_appraise <- function(object, nbins = 10L, B = 200L, seed = 1L, ...) {
  op <- par(mfrow = c(2, 3), mar = c(4, 4, 3, 1), cex = 0.8); on.exit(par(op))
  u <- lum_rqr(object, TRUE, seed); z <- qnorm(pmin(pmax(u, 1e-6), 1 - 1e-6))
  P <- lum_fitted(object, TRUE)
  ...
  cal <- lum_calibration(object, nbins, B, seed)
...
#'
#' Routes `performance::check_model()` to [lum_appraise()], so the panels are
#' the ones built for this model rather than generic ones that would not apply.
#'
#' @param x A fitted `"lum_model"` object.
#' @param ... Passed to [lum_appraise()].
#' @return Invisibly, the result of [lum_appraise()].
#' @export
check_model.lum_model <- function(x, ...) lum_appraise(x, ...)
...
#' @seealso [lum_check_omitted()].
#' @export
lum_check_covariate <- function(object, x, name = NULL, nbin = 5L, B = 30L,
  ...
  r <- lum_rqr_test(object, B = B, ncores = ncores, seed = seed,
...
#'
#' Runs [lum_check_covariate()] over every variable in a data frame that the
...
#' @export
lum_check_omitted <- function(object, data, vars = NULL, B = 30L, ncores = 1L,
  ...
  res <- lapply(cand, function(v)
    lum_check_covariate(object, data[[v]], name = v, B = B, ncores = ncores,
