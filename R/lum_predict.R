...
#'
#' A smooth is stored in a reparameterised form (see [lum_smooth()]), so
...
#' @keywords internal
#' @noRd
lum_smooth_design <- function(sob, newdata) {
...
#' @keywords internal
#' @noRd
lum_Bhat_term <- function(object, k, bvec = NULL) {
...
#' @keywords internal
#' @noRd
lum_newX <- function(object, newdata) {
  ...
  for (lab in names(object$smooths)) {
    sd_list[[lab]] <- lum_smooth_design(object$smooths[[lab]], newdata)
...
#' @keywords internal
#' @noRd
lum_eta <- function(object, nd, beta = NULL, bvec = NULL) {
  ...
  for (k in seq_along(object$re)) {
    ...
    ctb <- blk %*% lum_Bhat_term(object, k, bvec)
...
#' @keywords internal
#' @noRd
lum_softmax_J <- function(eta, Tc) { P <- exp(eta %*% t(Tc)); P / rowSums(P) }
...
#'
#' @param object A fitted `"lum_model"` object with `jointPrecision`.
...
#' @keywords internal
#' @noRd
lum_joint_draws <- function(object, nsim, seed) {
...
#' correlation described in `lum_joint_draws()` and distorts intervals around
#' smooths; a warning says so. [lum_model()] enables it automatically when the model
#' contains smooths.
#'
...
#'
#' @seealso [lum_model()], [lum_fitted()].
#' @export
predict.lum_model <- function(object, newdata = NULL,
  ...
  nd <- if (is.null(newdata)) list(X = object$X, smooths = NULL) else lum_newX(object, newdata)
  if (is.null(newdata) && length(object$smooths))
    nd$smooths <- lapply(object$smooths, lum_smooth_design, newdata = object$model)
  ...
  if (marginal && length(gk)) {                    # fixed RE draws: common random numbers
    ...
    draws <- lapply(gk, function(k) {
      ...
      else Z %*% lum_msqrt(object$Sigma[[k]])
    })
  }
  ...
  point <- function(beta, bvec = NULL) {
    ...
    for (m in seq_len(ndraw)) {
      ...
      P <- P + if (multinom) lum_softmax_J(sweep(eta, 2L, sh, `+`), Tc)
  ...
  jd <- if (!is.null(object$jointPrecision)) lum_joint_draws(object, nsim, seed + 1L) else NULL
  if (is.null(jd)) {
