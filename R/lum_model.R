...
##
##   lum_model(y ~ x1 + x2 + (1 | subj) + (1 + time | site) + s(x), data = dd)
##
## Random-effect bars are parsed with lme4::findbars; smooths with
## mgcv::interpret.gam, then reparameterised by lum_smooth() into fixed
...
#' t tests on `n - p` degrees of freedom and F tests in [lum_anova()], rather
...
#'   contains smooths, which is when [predict.lum_model()] needs it.
#' @param na.action How to handle missing values; default [stats::na.omit()].
#'
#' @return An object of class `"lum_model"`. Beyond the elements listed in
#'   [lum_fit()], a formula fit also stores `call`, `terms`, `xlev`,
#'   `contrasts`, the model frame and the smooth objects -- everything needed to
#'   rebuild a reference grid for [predict.lum_model()] and for `emmeans` or
#'   `marginaleffects`.
#'
#' @examples
#' \dontrun{
...
#'
#' fit <- lum_model(y ~ x1 + grp + (1 | subj), data = dd)
#' summary(fit)
#' lum_anova(fit, type = 3)
#' head(predict(fit))
#' }
#'
#' @seealso [summary.lum_model()], [lum_anova()], [predict.lum_model()],
#'   [lum_appraise()], [lum_pb_lrt()].
#' @rdname lum_model
#' @export
lum_model <- function(formula, ...) {
  cl <- match.call()
  use_formula <- inherits(formula, "formula")
  fn_name <- if (use_formula) "lum_model_formula" else "lum_fit"
  fn <- if (use_formula) lum_model_formula else lum_fit
  ...
  ## package is installed, because lum_model_formula() is internal and the caller
...
#' Formula front end
#'
#' Parses the formula, builds the design matrices and calls [lum_fit()]. Called
#' by [lum_model()]; documented separately only because [lum_model()] dispatches to it.
#'
#' Random-effect bars are extracted with `lme4::findbars()` and smooths with
#' `mgcv::interpret.gam()`, then reparameterised by `lum_smooth()` so that the
#' unpenalised part of each smooth joins the fixed effects and the penalised part
#' becomes a random term.
#'
#' @return An object of class `"lum_model"`.
#' @rdname lum_model
#' @export
lum_model_formula <- function(formula, data, family = "gaussian",
  ...
  fit <- lum_fit(X, yi, J, re_list, re_struct = re_struct, ar = ar,
...
#'
#' @param x,object,formula A fitted `"lum_model"` object.
#' @param ... Unused.
#' @return The model terms, formula, model frame, model matrix or original call.
#' @rdname lum_model-accessors
#' @export
terms.lum_model        <- function(x, ...) x$terms
...
#' @rdname lum_model-accessors
#' @export
formula.lum_model      <- function(x, ...) x$formula
...
#' @rdname lum_model-accessors
#' @export
model.frame.lum_model  <- function(formula, ...) formula$model
...
#' @rdname lum_model-accessors
#' @export
model.matrix.lum_model <- function(object, ...) {
  X <- object$X
...
#' @rdname lum_model-accessors
#' @export
getCall.lum_model <- function(x, ...) x$call
