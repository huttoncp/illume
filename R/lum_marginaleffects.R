...
#' @keywords internal
#' @noRd
lum_rebuild <- function(object, par) {
  ...
  for (k in seq_len(K)) {
    ...
    if (object$ty[k] == "rr") {
      L <- lum_mkLam_num(th, C, object$rk[k]); Lam[[k]] <- L; Sig[[k]] <- L %*% t(L)
    } else {
      L <- if (object$ty[k] == "diag") lum_mkD_num(th, C) else lum_mkL_num(th, C)
      Sig[[k]] <- L %*% t(L)
    }
    if (object$dk[k] > 1L) {
      thd <- blk[object$npc[k] + seq_len(object$npd[k])]
      Ld <- lum_mkLd_num(thd, object$dk[k], object$dcor[k])
      Sd[[names(object$re)[k]]] <- Ld %*% t(Ld)
    }
  }
  names(Sig) <- names(Lam) <- names(object$re)
  if (!is.null(object$ar)) {
    La <- lum_mkL_num(object$opt$par[grepl("^ar:L\\[", pn)], C)   # pnames is in fill order
    Sig[["ar"]] <- La %*% t(La)
...
#'
#' @param model A fitted `"lum_model"` object.
...
#'   `lum_model.marginal` option, which is `TRUE`.
...
#' @export
get_coef.lum_model <- function(model, ...) coef(model, full = TRUE)
...
#' @rdname marginaleffects-methods
#' @export
set_coef.lum_model <- function(model, coefs, ...) lum_rebuild(model, coefs)
...
#' @rdname marginaleffects-methods
#' @export
get_vcov.lum_model <- function(model, vcov = NULL, ...) {
...
#' @rdname marginaleffects-methods
#' @export
get_predict.lum_model <- function(model, newdata = NULL, type = "response",
                             marginal = NULL, ndraw = 100L, ...) {
  if (is.null(marginal))
    marginal <- isTRUE(getOption("lum_model.marginal", TRUE))   # PA is the default
...
#'
#' Call once per session before using `marginaleffects` with an `"lum_model"` fit.
#'
...
#' happens, so the methods would never be reached. This also adds `"lum_model"` to
...
#'   invisibly.
#' @export
lum_register_marginaleffects <- function() {
  ...
  for (g in c("get_coef", "set_coef", "get_vcov", "get_predict"))
    try(registerS3method(g, "lum_model", get(paste0(g, ".lum_model")), envir = ns), silent = TRUE)
  ...
  if (!"lum_model" %in% cur) options(marginaleffects_model_classes = c(cur, "lum_model"))
  invisible(TRUE)
}
