...
#' @rdname AIC.lum_model
#' @export
AIC.lum_model <- function(object, ..., k = 2) {
  lum_ic_warn(object)
  ll <- logLik(object)
...
#' @rdname AIC.lum_model
#' @export
BIC.lum_model <- function(object, ..., n = c("obs", "groups")) {
  lum_ic_warn(object); n <- match.arg(n)
  ll <- logLik(object)
...
#' @export
lum_fitted <- function(object, conditional = TRUE) {
  Tc <- contr.sum(object$J)
  eta <- lum_eta_hat(object, conditional)
  if (object$C > 1L) {
...
#'
#' The linear predictor behind [lum_fitted()], before the inverse link. Kept
...
#'
#' @param object A fitted `"lum_model"` object.
#' @param conditional Logical, as in [lum_fitted()].
...
#' @export
lum_scores <- function(object, conditional = TRUE) {
  P <- lum_fitted(object, conditional); y <- object$y; N <- nrow(P)
...
#' @keywords internal
#' @noRd
lum_null_ll <- function(object, restarts = 1L) {
  ...
  rl <- lum_re_list_of(object)[gi]
  f <- try(lum_fit(object$X[, keep, drop = FALSE], object$y, object$J, rl,
...
#' R-squared, and the scoring rules from [lum_scores()].
#'
...
#' @export
model_performance.lum_model <- function(model, metrics = "all", ..., verbose = TRUE) {
  ll <- logLik(model); df <- attr(ll, "df"); n <- attr(ll, "nobs")
  sc <- lum_scores(model, conditional = TRUE)
  ll0 <- lum_null_ll(model)
  out <- data.frame(
    AIC = AIC(model),
