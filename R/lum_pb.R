...
#' @seealso [lum_anova()].
#' @export
lum_pb_lrt <- function(object, term, B = 200L, ncores = 1L, seed = 1L,
                        restarts = 1L, verbose = TRUE) {
  if (is.null(object$assign))
    stop("lum_pb_lrt() needs a model fitted through the formula interface")
  ...
  t0 <- proc.time()[3]
  f0 <- lum_refit_drop(object, j, restarts = 2L)
  ...
  ys <- lum_simulate(f0, B, seed)

  keep <- is.na(object$assign) | object$assign != j
  X1 <- object$X; X0 <- object$X[, keep, drop = FALSE]
  J <- object$J; rl <- lum_re_list_of(object); rs <- object$re_struct
