...
#'
#' @param o A `"lum_model"` object with a binomial family.
#' @return A one-line character description.
#' @keywords internal
#' @noRd
lum_bin_target <- function(o) {
...
#' @keywords internal
#' @noRd
lum_fmt_corr <- function(S, labs, digits = 3) {
...
#'
#' @param object,x A fitted `"lum_model"` object.
...
#'   printed in full.
#' @param ... Unused.
#' @return `summary()` returns an object of class `"summary.lum_model"`; its print
#'   method returns it invisibly.
#' @seealso [lum_coef_table()], [lum_appraise()].
#' @rdname summary.lum_model
#' @export
summary.lum_model <- function(object, ...) {
  structure(list(object = object), class = "summary.lum_model")
}
...
#' @rdname summary.lum_model
#' @export
print.summary.lum_model <- function(x, digits = 4, max_corr_dim = 6L, ...) {
  ...
  if (fam == "binomial")
    writeLines(paste0("        ", lum_bin_target(o)))
  ...
  for (k in seq_along(o$re)) {
    ...
    S <- o$Sigma[[k]]
    if (C <= max_corr_dim) {
      print(lum_fmt_corr(S, o$ylevels[seq_len(C)], 3))
    } else {
      cat("   category SDs: ",
...
#' @return `x`, invisibly.
#' @export
print.lum_model <- function(x, ...) {
  cat(sprintf("lum_model fit: %d categories, %d obs, %d fixed + %d covariance parameters%s\n",
