...
## lum_model()'s likelihood has none: the response enters as a fixed indicator matrix
...
## reuses lum_simulate(), which the package needs anyway for population-averaged
...
#' @return A list suitable for passing back to [lum_fit()].
#' @keywords internal
#' @noRd
lum_re_list_of <- function(fit) {
...
#' diagnostic, and [lum_consistency()]. Simulation is how this package builds
...
#'   category codes.
#' @seealso [lum_consistency()], [lum_pb_lrt()], [lum_appraise()].
#' @export
lum_simulate <- function(fit, nsim = 1L, seed = NULL) {
  ...
  for (s in seq_len(nsim)) {
    ...
    for (k in seq_len(K)) {
      ...
      Bc <- if (isrr) diag(1, w) else lum_msqrt(fit$Sigma[[k]])
      B <- matrix(0, nl * d, w)
      for (g in seq_len(nl)) {
    ...
    if (!is.null(fit$ar)) {
      ar <- fit$ar; La <- lum_msqrt(fit$Sigma[["ar"]]); rho <- fit$rho
...
#' @keywords internal
#' @noRd
lum_summarise_fit <- function(fit) {
...
#' @export
lum_consistency <- function(fit, B = 50L, seed = 1L, ncores = 1L, verbose = TRUE) {
  t0 <- proc.time()[3]
  ## Simulated up front on the master, so the parallel section is deterministic
  ## given its input: results do not depend on ncores.
  ys <- lum_simulate(fit, B, seed)
  p0 <- lum_summarise_fit(fit)
  TH <- matrix(NA_real_, length(p0), B, dimnames = list(names(p0), NULL))
  rr <- lum_refit_many(fit, ys, ncores = ncores, restarts = 2L, verbose = verbose)
