## illume::lum_model() -- multinomial linear mixed model, matrix-normal core.
...
#' entries, because the upper and lower triangles mirror each other. `lum_nrr()`
...
#' @keywords internal
#' @noRd
lum_ncov <- function(C) as.integer(C * (C + 1L) / 2L)              # unstructured
...
#' @keywords internal
#' @noRd
lum_nrr  <- function(C, r) as.integer(C * r - r * (r - 1L) / 2L)   # reduced rank
...
#'
#' `lum_mkL()` builds a full unstructured factor, `lum_mkD()` a diagonal one (no
#' correlation between categories), and `lum_mkLam()` a C-by-r loadings matrix for a
...
#' @param r Integer. Rank; `lum_mkLam()` only.
...
#' @keywords internal
#' @noRd
lum_mkL <- function(v, C) {                       # unconstrained -> lower Cholesky
...
#' @keywords internal
#' @noRd
lum_mkD <- function(v, C) {                       # diagonal "Cholesky"
  Lv <- AD(numeric(C * C))
...
#' @keywords internal
#' @noRd
lum_mkLam <- function(v, C, r) {                  # C x r loadings, lower-trapezoidal
...
#' @keywords internal
#' @noRd
lum_mkLd <- function(v, d, d_cor = TRUE) {
...
#' to resolve the Kronecker scale indeterminacy described in [lum_mkLd()].
#'
...
#' @keywords internal
#' @noRd
lum_npar_d <- function(d, d_cor = TRUE)
  if (d > 1L) { if (d_cor) lum_ncov(d) - 1L else d - 1L } else 0L

#' Plain-numeric versions of the covariance factor builders
#'
#' Behave identically to [lum_mkL()] and its relatives but operate on ordinary
...
#' @param d_cor Logical; `lum_mkLd_num()` only.
#' @return A numeric matrix.
#' @keywords internal
#' @noRd
lum_mkL_num <- function(v, C) {
  L <- matrix(0, C, C); k <- 1L
...
#' @keywords internal
#' @noRd
lum_mkLd_num <- function(v, d, d_cor = TRUE) {
...
#' @keywords internal
#' @noRd
lum_mkD_num   <- function(v, C) diag(exp(v), C, C)
...
#'
#' Their fill order **must** mirror the matching matrix builders ([lum_mkL()],
#' [lum_mkD()], [lum_mkLam()], [lum_mkLd()]). Were the two ever to drift apart, every
#' parameter would be silently mislabelled, which is worse than having no names
#' at all. [lum_fit()] therefore checks that the number of names matches the
...
#' @param d Integer. Number of within-group dimensions.
#' @param r Integer. Rank.
#' @param d_cor Logical; `lum_nm_ld()` only.
#' @return A character vector of parameter names.
#' @keywords internal
#' @noRd
lum_nm_tri  <- function(pre, C) { o <- character(0)
...
#' @keywords internal
#' @noRd
lum_nm_diag <- function(pre, C) sprintf("%s:logsd[%d]", pre, seq_len(C))
...
#' @keywords internal
#' @noRd
lum_nm_lam  <- function(pre, C, r) { o <- character(0)
...
#' @keywords internal
#' @noRd
lum_nm_ld   <- function(pre, d, d_cor) { o <- character(0)
...
#'
#' Report how many parameters a structure costs (`lum_str_npar()`), how many latent
#' values it needs per group (`lum_str_width()`), and a short printing label
#' (`lum_str_label()`).
#'
...
#' @param C Integer. Number of category dimensions.
#' @return An integer, or a character label for `lum_str_label()`.
#' @keywords internal
#' @noRd
lum_str_npar  <- function(s, C) switch(s$type, us = lum_ncov(C), diag = as.integer(C), rr = lum_nrr(C, s$rank))
...
#' @keywords internal
#' @noRd
lum_str_width <- function(s, C) switch(s$type, us = C, diag = C, rr = s$rank)
...
#' @keywords internal
#' @noRd
lum_str_label <- function(s) if (s$type == "rr") sprintf("rr(%d)", s$rank) else s$type
...
#' @keywords internal
#' @noRd
lum_rec_rank <- function(C, nl) {
  r <- 0L
  for (rr in seq_len(max(1L, C - 1L))) if (lum_nrr(C, rr) <= nl / 6) r <- rr
  r
}
...
#' smooth, after reparameterisation by `lum_smooth()`).
#'
...
#' @keywords internal
#' @noRd
lum_norm_re <- function(re_list, N) lapply(re_list, function(e) {
...
#'   [predict.lum_model()] needs it to rebuild the basis at new covariate values).
#' @references
...
#' @keywords internal
#' @noRd
lum_new_checks <- function() data.frame(check=character(), status=character(),
...
#' @keywords internal
#' @noRd
lum_add_check <- function(ck, check, status, detail, cause = "", suggestion = "")
...
#' @param re Normalised random-effects list from [lum_norm_re()].
...
#' @keywords internal
#' @noRd
lum_precheck <- function(y, J, re, re_struct, ar = NULL, weights = NULL,
                     family = NULL) {
  C <- J - 1L
  ...
  N <- sum(weights); ck <- lum_new_checks()
  ...
  ck <- lum_add_check(ck, "category_counts",
  ...
  ck <- lum_add_check(ck, "weights_type", wst,
    ...
    if (wst != "OK") paste0(
      ...
      "; lum_model() treats weights as FREQUENCIES. Applied as sampling weights they bias the fixed effects AND understate the standard errors") else "",
    if (wst != "OK")
  ...
  for (nm in names(re)) {
    ...
    np <- lum_str_npar(s, C) + lum_npar_d(e$d, isTRUE(s$d_cor))
    w  <- lum_str_width(s, C); lat[nm] <- nl * e$d * w
    ratio <- nl / np
      ...
      if (s$type %in% c("us", "diag")) {
        ...
        sug <- if (r >= 1)
          sprintf("use rr(%d) for this term: %d parameters instead of %d, and latent width %d instead of %d",
                  r, lum_nrr(C, r), lum_str_npar(s, C), r, C)
    ...
    ck <- lum_add_check(ck, paste0("re_levels[", nm, "]"), st,
      sprintf("%d %s for %d covariance parameters (%.1f per parameter); %s, C = %d%s",
              nl, unit, np, ratio, lum_str_label(s), C,
    ...
    if (e$kind != "basis") {          # a basis term has no grouping levels
      ope <- N / nl
      ck <- lum_add_check(ck, paste0("obs_per_level[", nm, "]"),
  ...
  if (st != "OK") {
    ...
    if (big %in% names(re)) {
      ...
      sug <- sprintf("total latent budget for %g observations is about %d; term '%s' contributes %d (%s, width %d). rr(%d) would cut it to %d",
                     N, as.integer(target), big, lat[big], lum_str_label(s),
                     lum_str_width(s, C), keep, as.integer(e$nl * e$d * keep))
  ...
  ck <- lum_add_check(ck, "latent_budget", st,
  ...
  if (!is.null(ar)) {
    ...
    ck <- lum_add_check(ck, "obs_per_ar_latent",
...
#' @keywords internal
#' @noRd
lum_postcheck <- function(opt, obj, sdr, C, has_ar, pre, Sig, Sigd, re_struct, kinds = NULL,
                      pnames = NULL) {
  kind_of <- function(nm) if (!is.null(kinds) && nm %in% names(kinds)) kinds[[nm]] else "group"
  ck <- lum_new_checks(); g <- max(abs(obj$gr(opt$par)))
  ck <- lum_add_check(ck, "optimizer",
  ...
  if (length(gsd)) {
    bad <- gsd < 1e-3
    ck <- lum_add_check(ck, "variance_boundary", if (any(bad)) "WARN" else "OK",
  ...
  for (nm in names(Sig)) {
    ...
    ck <- lum_add_check(ck, paste0("smooth_shrinkage[", nm, "]"),
  ...
  for (nm in names(Sig)) {
    ...
    ck <- lum_add_check(ck, paste0("sigma_rank[", nm, "]"),
  ...
  for (nm in names(Sigd)) {
    ...
    ck <- lum_add_check(ck, paste0("sigma_within[", nm, "]"), st, dt, wy, tr)
  }
  if (has_ar) {
    rho <- est[nmv == "rho"]
    lat <- pre$status[pre$check == "obs_per_ar_latent"]
    ck <- lum_add_check(ck, "rho_boundary",
  ...
  if (!is.null(cf) && all(is.finite(cf)) && all(diag(cf) > 0)) {
    ...
    ck <- lum_add_check(ck, "parameter_aliasing",
    ...
    ck <- lum_add_check(ck, "parameter_aliasing", "INCONCLUSIVE",
...
#' @keywords internal
#' @noRd
lum_print_checks <- function(ck, title) {
...
#'
#' The computational engine. Most users should call [lum_model()] with a formula
...
#' [lum_consistency()] to test it directly on your own fit.
#'
...
#'   [lum_family()]. See [lum_family()] for what each one assumes.
...
#'   `list(basis =)` (a smooth, from `lum_smooth()`).
...
#'   parameters. Needed by [predict.lum_model()] to propagate uncertainty in penalised
#'   smooth coefficients; [lum_model()] switches it on automatically when the model
#'   contains smooths.
#'
#' @return An object of class `"lum_model"`: a list whose most useful elements are
...
#' 805--827.
#'
#' @seealso [lum_model()] for the formula interface, [summary.lum_model()],
#'   [lum_consistency()].
#' @export
lum_fit <- function(X, y, J = NULL, re_list, re_struct = NULL, ar = NULL,
                     ylevels = NULL, weights = NULL, family = "multinomial",
                     verbose = TRUE, restarts = 3L, joint = FALSE) {
  fam <- if (is.list(family)) family else lum_family(family)
  ...
  pre <- lum_precheck(y, J, re, re_struct, ar, weights, fam)
  if (verbose) lum_print_checks(pre, "pre-fit data checks")
  ...
  if (fam$name == "multinomial") {
    ...
    lum_check_response(y, fam)
    yobs <- as.numeric(y)
  }
  K   <- length(re)
  nlk <- vapply(re, `[[`, 1L, "nl"); dk <- vapply(re, `[[`, 1L, "d")
  wk  <- vapply(names(re), function(nm) lum_str_width(re_struct[[nm]], C), 1L)
  npc <- vapply(names(re), function(nm) lum_str_npar(re_struct[[nm]], C), 1L)
  dcor <- vapply(re_struct, function(s) isTRUE(s$d_cor), TRUE)
  npd  <- as.integer(mapply(lum_npar_d, dk, dcor))
  ...
  pnames <- c(
    ...
    unlist(lapply(seq_len(K), function(k) {
      nm <- names(re)[k]
      a <- switch(ty[k], us = lum_nm_tri(nm, C), diag = lum_nm_diag(nm, C),
                  rr = lum_nm_lam(nm, C, rk[k]))
      if (dk[k] > 1L) a <- c(a, lum_nm_ld(nm, dk[k], dcor[k]))
      a
    })))
  if (fam$n_disp > 0L) pnames <- c(pnames, fam$disp_names)
  if (!is.null(ar)) pnames <- c(pnames, lum_nm_tri("ar", C), "ar:rho_raw")
  ...
  f <- function(pars) {
    ...
    for (k in seq_len(K)) {
      ...
      if (ty[k] == "rr") {                         # columns are already N(0,1)
        Lam <- lum_mkLam(th[1:npc[k]], C, rk[k]); lcld <- 0
        M <- lapply(1:d, rows)
        S <- Lam %*% t(Lam)
      } else {
        L <- if (ty[k] == "diag") lum_mkD(th[1:npc[k]], C) else lum_mkL(th[1:npc[k]], C)
      ...
      if (d == 1L) {
        ...
        Ld <- lum_mkLd(th[(npc[k] + 1L):(npc[k] + lum_npar_d(d, dcor[k]))], d, dcor[k])
    ...
    if (has_ar) {
      La <- lum_mkL(lchol_ar, C); rho <- tanh(rho_raw)
  ...
  if (!is.null(ar)) {
    pars$lchol_ar <- rep(0, lum_ncov(C)); pars$rho_raw <- 0.5
    pars$B_ar <- matrix(0, ar$n_group * ar$Tt, C); rnd <- c(rnd, "B_ar")
  }
  t0 <- proc.time()[3]
  obj <- MakeADFun(f, pars, random = if (length(rnd)) rnd else NULL,
                   silent = TRUE)
  ## guard: if a fill order in nm_* ever drifts from lum_mkL/lum_mkD/lum_mkLam/lum_mkLd, the
  ...
  Sig <- lapply(seq_len(K), function(k) {
    ...
    if (ty[k] == "rr") { L <- lum_mkLam_num(th, C, rk[k]); Lams[[k]] <<- L; L %*% t(L) }
    else { L <- if (ty[k] == "diag") lum_mkD_num(th, C) else lum_mkL_num(th, C); L %*% t(L) }
  })
  names(Sig) <- names(re)
  Sigd <- list()
  for (k in seq_len(K)) if (dk[k] > 1L) {
    th <- thv[(toff[k] + 1L):toff[k + 1L]][(npc[k] + 1L):(npc[k] + lum_npar_d(dk[k], dcor[k]))]
    Ld <- lum_mkLd_num(th, dk[k], dcor[k]); Sigd[[names(re)[k]]] <- Ld %*% t(Ld)
  }
  if (!is.null(ar)) { La <- lum_mkL_num(pe[pn == "lchol_ar"], C); Sig[["ar"]] <- La %*% t(La) }

  post <- lum_postcheck(opt, obj, sdr, C, !is.null(ar), pre, Sig, Sigd, re_struct,
                    kinds = as.list(vapply(re, `[[`, "", "kind")), pnames = pnames)
  if (verbose) lum_print_checks(post, "post-fit convergence checks")
  st <- c(pre$status, post$status)
  if (verbose) {
    cat("\nstructure: ", paste(sprintf("%s = %s%s (%d par, %d latent)", names(re),
        vapply(re_struct, lum_str_label, ""),
  ...
  ## give.  Recorded here; vcov(), lum_coef_table() and lum_anova() act on it.
  ...
  structure(list(obj = obj, opt = opt, sdr = sdr, checks = rbind(pre, post),
                 ...
                 ok = !any(st == "FAIL")), class = "lum_model")
}
