...
#' @keywords internal
#' @noRd
lum_pearson_ovr <- function(object) {
  P <- lum_fitted(object, TRUE); N <- nrow(P)
...
#' @keywords internal
#' @noRd
lum_resid_acf <- function(z, grp, tim, maxlag = 8L, min_pairs = 20L) {
  ord <- order(grp, tim)
...
#' structures.)
#' @seealso [lum_appraise()], [lum_check_omitted()].
#' @export
lum_check_ar <- function(object, time, group, maxlag = 8L, B = 30L,
  ...
  acf_all <- function(fit) {
    ...
    vapply(seq_len(ncol(R)), function(j) lum_resid_acf(R[, j], grp, tim, maxlag),
  ...
  reps <- lum_lapply(cl, seq_len(B), one)
