...
## table that answers the wrong question, so lum_model registers its own method.
##
...
#' @keywords internal
#' @noRd
lum_term_idx <- function(object, t) {
...
#' @keywords internal
#' @noRd
lum_relatives_of <- function(mt, j) {
  fo <- attr(mt, "factors")
...
#' @return A refitted `"lum_model"` object.
#' @keywords internal
#' @noRd
lum_refit_drop <- function(object, drop_terms, restarts = 2L) {
  keep <- is.na(object$assign) | !(object$assign %in% drop_terms)
  ## lum_fit() defaults to the multinomial family, so an internal refit that
  ## does not pass the fitted family is silently a DIFFERENT model.  Weights
  ## must carry over for the same reason.
  f <- lum_fit(object$X[, keep, drop = FALSE], object$y, object$J,
                lum_re_list_of(object), object$re_struct, object$ar,
...
#' @keywords internal
#' @noRd
lum_drop_summary <- function(object, drop_terms, restarts = 2L) {
  f <- try(lum_refit_drop(object, drop_terms, restarts), silent = TRUE)
...
#' @param restarts Integer. Optimiser restarts.
#' @return A list of [lum_drop_summary()] results.
#' @keywords internal
#' @noRd
lum_refit_drops <- function(object, drop_sets, ncores = 1L, restarts = 2L) {
  X <- object$X; y <- object$y; J <- object$J; asg <- object$assign
  rl <- lum_re_list_of(object); rs <- object$re_struct; arr <- object$ar
  yl <- object$ylevels; tl <- object$term_labels
  fm <- object$family; wt <- object$weights; Cc <- object$C
  ## lum_drop_summary() calls lum_re_list_of(), which needs $re; hand over the already
  ...
  one <- function(i) {
    ...
    lum_drop_summary(stub, drop_sets[[i]], restarts)
  }
  if (!is.null(cl)) parallel::clusterExport(cl, "rl_re", envir = environment())
  lum_lapply(cl, seq_along(drop_sets), one)
}
...
#' @keywords internal
#' @noRd
lum_wald_block <- function(b, V, idx) {
...
#' for small samples use [lum_pb_lrt()] instead, since both rely on
#' large-sample approximations.
#'
#' @param object A fitted `"lum_model"` object, fitted through the formula interface.
...
#'   than assumed known (see [lum_model()]). A model with no terms to test, such
...
#' 851--853.
#' @seealso [lum_pb_lrt()], [lum_coef_table()].
#' @export
lum_anova <- function(object, type = 3, test = c("Wald", "LRT"),
  ...
  if (is.null(object$assign))
    stop("no term map: lum_anova() needs a model fitted through the formula interface")
  ...
  rel <- lapply(seq_len(nt), function(j) lum_relatives_of(mt, j))
  ...
  fits <- if (length(need)) lum_refit_drops(object, need, ncores, restarts) else list()
  ...
  rows <- lapply(seq_len(nt), function(j) {
    ...
    if (test == "Wald") {
      if (!nzchar(keyM1[j])) {
        stat <- lum_wald_block(b_full, V_full, lum_term_idx(object, j))
      } else {
        f1 <- fits[[keyM1[j]]]
        ...
        stat <- lum_wald_block(f1$b, f1$V, lum_term_idx(stub, j))
      }
    } else {
...
#' car::Anova method
#'
#' Lets `car::Anova(fit, type = 3)` dispatch to [lum_anova()], so it uses the
#' correct joint blocking across categories instead of `car`'s default, which
#' would test only one category.
#'
#' @param mod A fitted `"lum_model"` object.
#' @param type `"II"`, `"III"`, `2` or `3`.
#' @param test.statistic Ignored; present for compatibility.
#' @param ... Passed to [lum_anova()].
#' @return An `"anova"` data frame.
#' @export
Anova.lum_model <- function(mod, type = c("II", "III", 2, 3), test.statistic = "Chisq", ...) {
  type <- as.character(type)[1]
  lum_anova(mod, type = if (type %in% c("3", "III")) "3" else "2", ...)
}
