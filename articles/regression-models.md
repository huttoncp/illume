...
A smooth is named as mgcv names it. `s(x)` is `"s(x)"`, and a smooth
with a numeric `by`, `s(x, by = z)`, is `"s(x):z"`. Its unpenalised
columns among the fixed effects carry the name with `.f1`, `.f2`, …
after it (`"s(x):z.f1"`), and its penalised part is the random term of
that name. A smooth with a numeric `by` used to be called `"s(x)"`, with
columns `"s(x).f1"` and `"s(x).f2"`. Beside a plain `s(x)` that
collided, and the fit lost one of the two. The section “How smooth terms
are named” in
[`?ilm_model`](https://huttoncp.github.io/illume/reference/ilm_model.md)
has the whole scheme.

A numeric `by` variable is not also entered on its own. `s(x, by = z)`
is not centred, so it already carries `z`’s main effect, and
`y ~ z + s(x, by = z)` stops and says so.

A random term’s covariance is set through `re_struct`: a list with one
element per term, named by its grouping variable, each element itself a
list. Two settings go in it. `d_cor = FALSE` drops the correlation
between a random intercept and its slopes, in any family. `type` sets
the covariance across a multinomial outcome’s categories – `"us"` by
default, `"diag"`, or `"rr"` with a `rank` – and matters only there,
since every other family has a single linear predictor:
...
`family` defaults to `"auto"`, which reads the family off the response
and says which it chose and on what evidence:
...
unordered factor with three or more levels is multinomial and an ordered
one ordinal; values strictly between 0 and 1 are beta. Whole numbers are
the hard case, because a count and a measurement recorded to the nearest
unit look alike: whole numbers that reach down to 0 or 1 are read as a
count, and ones that never come near zero – a blood pressure, a height –
as a measurement. The range goes into the message either way, so a
misreading shows at a glance.

What the response cannot settle is asked about rather than guessed: a
proportion that touches 0 or 1, or comes with weights that may be
numbers of trials; a two-valued variable coded other than 0/1; a
survival time, where the family is a modelling decision; a date. The
choice is written into `fit$call`, so anything that refits the model
keeps it instead of guessing again on simulated data, and
[`summary()`](https://rdrr.io/r/base/summary.html) marks the family as
inferred. It is a starting point, not a verdict – a count that is
overdispersed is still a count, and `family = "nbinom"` is a choice only
the diagnostics can prompt.
...
Excess zeros have a part of their own: `ziformula = ~ 1` adds a
zero-inflation probability, and `zi_type = "hurdle"` separates whether
the response clears zero from how far past it goes – see
[`?ilm_model`](https://huttoncp.github.io/illume/reference/ilm_model.md)
for which is which, and
[`ilm_check_zeros()`](https://huttoncp.github.io/illume/reference/ilm_check_zeros.md)
for whether either is needed. Proportions strictly between 0 and 1 take
`family = "beta"`, ordered categories the ordinal families, and times to
an event the survival families below. Tweedie is out of scope; `glmmTMB`
covers it well.
...
Written as `~ day | subj`, the time and the unit are read from the data
after incomplete rows are dropped, so they cannot fall out of step with
the response, as vectors taken from the whole data frame can. All three
...
A CAR(1) process wanders and comes back: however far it strays, it is
pulled towards its mean. A level that drifts and stays where it drifted
to is a random walk, which
[`ilm_rw1()`](https://huttoncp.github.io/illume/reference/ilm_rw1.md)
fits – the local-level model of time-series analysis. Each unit’s walk
is zero at its first time, so the fixed effects give its starting level,
and from there it moves by changes whose variance is proportional to the
time elapsed:
...
maximum likelihood or by REML.
[`ilm_cells()`](https://huttoncp.github.io/illume/reference/ilm_cells.md)
lists the fitted walk’s cells: each unit’s times, which is its last, and
where the fit holds each value.
...
Each effect is said over a change a reader can picture: across the
middle half of a numeric predictor, level by level for a factor, with
the model’s predictions at both ends, averaged over the rows it was
fitted to – “across the middle half of age, predicted income is 49.4 at
29 against 64.7 at 54: 15.2 higher (95% interval 12.3 to 18.1)”. A
coefficient of 0.61 says nothing until the reader knows what a unit of
age is and how much age varies; the middle half answers both. The
strength of evidence is the term’s joint test, so a factor with several
levels, or a predictor in a multinomial model, gets one verdict.
