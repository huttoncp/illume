...
- **Merged:** PRs \#6 to \#11: the 0.0.8.9000 bump, the interpretation
  work, the remedies and REML documentation, the joint-draw order (D1),
  averaging over every random term (D2 to D4) and the quadrature and
  scenario follow-up. Both packages were rebuilt from main and installed
  into Craig’s R 4.4 and R 4.6.1 libraries after \#11.
- **In review:** `random-walk`, which adds
  [`ilm_rw1()`](https://huttoncp.github.io/illume/reference/ilm_rw1.md),
  the by-name form `~ time | group` for all three structures over time,
  and
